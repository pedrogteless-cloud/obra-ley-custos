...
function Gastos() {
  ...
  const [notaFiltro, setNotaFiltro] = useState<"todas" | "com" | "sem">("todas");
  ...
  const filtrado = lancs.filter((l) => {
    ...
    if (busca) {
      ...
      return (
        l.descricao.toLowerCase().includes(q) || (l.fornecedor ?? "").toLowerCase().includes(q)
      );
    }
    return true;
  });
    ...
    <AppLayout titulo="Gastos" subtitulo={`${filtrado.length} lançamentos`}>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 mb-3 -mx-4 px-4">
        {(
          [
            { v: "todas", label: "Todos" },
            { v: "com", label: "📄 Com papel" },
            { v: "sem", label: "✏️ Sem papel" },
          ] as const
        ).map((opt) => {
          const ativo = notaFiltro === opt.v;
          return (
            <button
              key={opt.v}
              onClick={() => setNotaFiltro(opt.v)}
              className={`shrink-0 px-3 h-8 rounded-full text-xs font-medium transition ${
                ativo
            ...
            return (
              <div key={l.id} className="rounded-2xl bg-card border border-border p-3 flex gap-3">
                <div
                  ...
                  style={{
                    backgroundColor: `color-mix(in oklch, var(--${cat.color}) 15%, transparent)`,
                  }}
                >
                  {l.comprovante_url ? (
                    <ComprovanteImg
                      path={l.comprovante_url}
